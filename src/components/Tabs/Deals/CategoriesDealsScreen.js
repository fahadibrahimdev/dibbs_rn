...
import {
  removeProduct,
  saveProduct,
  searchDealsViaCategory,
  setSearchProducts,
} from '../../../redux/actions/productActions';
const Image = createImageProgress(FastImage);

class CategoriesDealsScreen extends Component {
  state = {
    //Alert State
  ...
  componentDidMount() {
    ...
    this.setState({categoryID: pCategoryID, categoryName: pCategoryName});

    this.props.searchDealsViaCategory('', pCategoryID);
  }

  componentDidUpdate(prevProps) {
    if (
      this.props.searchedDealsVIACategory.length !==
      prevProps.searchedDealsVIACategory.length
    ) {
    ...
    if (
      this.props.searchedDealsVIACategory.length ===
        prevProps.searchedDealsVIACategory.length &&
      this.props.allDealsVIACategoryFetched !==
        prevProps.allDealsVIACategoryFetched
    ) {
      this.startSearch(this.state.searchText);
    }
  }
  ...
  startSearch(searchText) {
    if (searchText === '') {
      this.setState({
        filteredData: this.props.searchedDealsVIACategory,
      });
    } else {
      const newFilteredData = this.props.searchedDealsVIACategory.filter(
        obj =>
  ...
  onRefresh = () => {
    this.props.searchDealsViaCategory('', this.state.categoryID);
  };
    ...
    return (
      <View
        ...
        <SafeAreaView style={{flex: 1}}>
          ...
          <View style={{flex: 1}}>
            {!this.props.isSearchingVIACategoryDeals &&
              this.props.searchedDealsVIACategory.length === 0 && (
                <View
                  style={{
                    width: '100%',
                    height: '100%',
            ...
            <FlatList
              ...
              refreshControl={
                <RefreshControl
                  ...
                  refreshing={this.props.isSearchingVIACategoryDeals}
...
const mapStateToProps = (state, ownProps) => {
  ...
  const {
    isSearchingVIACategoryDeals,
    allDealsVIACategoryFetched,
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    searchDealsViaCategory: (keyword, categoryID) =>
      dispatch(searchDealsViaCategory(keyword, categoryID)),
